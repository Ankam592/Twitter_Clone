            ...
            <div className="mt-4 w-50 h-90 flex flex-col justify-start items-center flex-wrap">
                {nav_links.map((nav_ele) => {
                    ...
                    return nav_ele.IsAuthenticated ? (
                        <div className={`${loc.pathname == nav_ele.route && "text-[#1DA1F2] border-l border-4px border-[#1DA1F2] rounded-lg border-[#1DA1F2] bg-[#D0E8FF]  "} pl-2 pr-2  w-37 h-10 flex justify-center hover:bg-[#C0DFFE] rounded-lg items-center`}>
                            {IconComponent && <IconComponent onClick={() => nav(nav_ele.route)} className="w-6 h-10 text-[#1DA1F2]" />}
                            <Button key={nav_ele.route}
